...
function repeatString(str, times) {
  return times < 0 ? 0 : str.repeat(times);
}
...
function countVowels(str) {
  return str
    .split('')
    .filter((char) => 'aeiouy'.includes(char.toLocaleLowerCase())).length;
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((char) => {
      ...
      return index === -1 ? char : rotated[index];
    })
    .join('');
}
...
function getCardId(value) {
  ...
  return suits.indexOf(suit) * 13 + cards.indexOf(cardValue);
}
